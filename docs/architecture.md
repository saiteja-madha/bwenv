...
Import parses and validates the complete dotenv input, lists once, builds a full-key index, and applies sorted upserts. It is fail-fast but not transactional because the official CLI exposes independent remote operations.

Run performs the same single-list and merge path, converts entries to a unique environment map, removes the machine access token, and starts a shell with inherited stdio. It waits for the child so exit status is portable across macOS, Linux, and Windows.
...
- Never pass `BWS_ACCESS_TOKEN` into a command started by `run`.
...
Pure domain and renderer tests cover validation, precedence, duplicates, stable output, and special values. Command tests inject a fake Bitwarden client and real in-memory stdio. Runner tests cover argument construction and masking without contacting Bitwarden. Cross-platform CI tests and builds all supported operating systems; the race detector runs on Linux.
